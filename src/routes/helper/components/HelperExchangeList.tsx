...
export default function ExchangesHistory() {
  ...
  const [exchangerData, setExchangerData] = useState(null);
  const user = User()
  ...
  useEffect(() => {
    ...
    getData(endPoints.profileHelper)
      .then(value => setExchangerData(value))
      .catch(() => setExchangerData({ employeeLocation: { id: null } }));
    //console.log(exchangerData.employeeLocation.name)
  }, []);

  useEffect(() => {
    if (exchangerData && exchangerData.employeeLocation && exchangerData.employeeLocation.id) {
      getData(endPoints.todayIsFinished + "/" + exchangerData.employeeLocation.id)
        .then(value => setDayEnded(value))
        .catch(() => setDayEnded(false));
    }
  }, [exchangerData]);
  ...
  const endDay = () => {
    putData(endPoints.endDay + "/" + exchangerData.employeeLocation.id)
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      {exchangerData?.employeeLocation && <ExchangerHeader title={"Intercambios de la sede \""+exchangerData?.employeeLocation?.name+"\""}>
